...
const COLORS = {
  ...
  sky:    { bg: 'bg-sky-50',    text: 'text-sky-700',    border: 'border-sky-100',    bar: 'bg-sky-500' },
}
      ...
      <div className="min-h-screen bg-sky-100">
        ...
        <main className="max-w-lg mx-auto px-4 pb-10">
          <div className="pt-6 mb-5">
            ...
            <p className="text-sm text-slate-400">Elige un tema, revisa la explicación, mira los videos y practica</p>
          </div>
                    ...
                    <Link key={t.id} href={'/escuela/' + query.id + '/tema/' + t.id}>
                      <div className={`card-hover press bg-white rounded-2xl border ${c.border} shadow-sm p-4 flex items-center gap-3 cursor-pointer anim-up`} style={{ animationDelay: (gi * 4 + ti) * 0.06 + 's', opacity: 0, animationFillMode: 'forwards' }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-slate-900 text-sm truncate mb-0.5">{t.title}</p>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-400">{(t.practice || []).length} ejercicios</span>
                            <span className="text-xs text-slate-400">· {(t.videos || []).length} videos</span>
                          </div>
                        </div>
