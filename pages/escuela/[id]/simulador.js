...
export default function SimuladorPage() {
  ...
  function startExam() {
    setCurrent(0); setAnswers({}); setSelected(null); setFeedback(false); setTimeLeft(2700)
    setPhase('exam')
      ...
      <div className="min-h-screen bg-sky-100">
        ...
        <main className="max-w-lg mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {[['❓', questions.length, 'Preguntas'], ['⏱️', '45 min', 'Tiempo'], ['📊', '3-5', 'Opciones'], ['🎯', '70%', 'Para pasar']].map(([ic, v, l]) => (
      ...
      <div className="min-h-screen bg-sky-100">
        ...
        <main className="max-w-lg mx-auto px-4 pb-6">
          <div className="flex items-center justify-between pt-4 mb-3">
            ...
            <span className={`px-3 py-1.5 rounded-xl font-mono font-black text-sm ${timeLeft <= 300 ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-900'}`}
              style={timeLeft <= 300 ? { animation: 'pulse 1s infinite' } : {}}>
              ⏱️ {fmt(timeLeft)}
            </span>
          </div>
